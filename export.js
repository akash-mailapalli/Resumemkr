...
export async function downloadPDF(elementId, fileName = "resume.pdf") {
  ...
  clone.style.paddingTop = element.style.paddingTop || "20px";
  clone.style.paddingBottom = element.style.paddingBottom || "20px";
  clone.style.paddingLeft = element.style.paddingLeft || "20px";
  clone.style.paddingRight = element.style.paddingRight || "20px";
  ...
  allClonedDescendants.forEach(el => {
    ...
    // 2. Ensure normal letter-spacing to prevent html2canvas character-splat overlapping bug
    // Setting it to normal lets the browser apply standard kerning which is gorgeous and prevents overlapping characters
    ...
    // 4. Force clean visible dark text colors ONLY for general container texts to prevent inheriting white body texts,
    // but preserve the original color of elements with specific styling class colors (like text-indigo-600, text-slate-400, etc.)
    // and especially preserve light/white text colors inside high-contrast components (like pill badges)
    const hasSpecificTextColor = classNameStr.match(/\btext-(indigo|slate|blue|emerald|amber|rose|violet|purple|pink|cyan|teal|sky|gray|zinc|neutral|stone|red|orange|yellow|fuchsia|white)-\d+/i) || classNameStr.includes("text-white") || el.style.color;
    if (!isHeading && el.tagName !== "SVG" && !el.closest("svg") && !hasSpecificTextColor) {
    ...
    if (resumeData.projects && resumeData.projects.length > 0) {
      ...
      resumeData.projects.forEach(proj => {
        ...
        if (proj.skills) {
          sections.projects.push(
            createTextParagraph([
              { text: `[${proj.skills}]`, color: secondaryHex, size: 18 }
            ], { spaceAfter: 40 })
          );
        }
        if (proj.description) {
